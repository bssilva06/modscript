...
**Author:** Ben Silva  
**Version:** 1.4  
**Status:** Draft — Reconciled with Devvit Web docs; billing model finalized as Model A (developer-provided global key with rate limits)  
...
**ModScript** is a Devvit mod tool that lives inside Reddit's native mod panel. It gives moderators a conversational AI interface — powered by Claude — to generate, explain, and audit their AutoModerator configuration without writing a single line of YAML or ever leaving Reddit.
...
- Claude generates valid, formatted AutoMod YAML
...
- **Explicit rewrite mode:** a clearly labeled "Rewrite full config" button is available behind a confirmation dialog for mods who want Claude to restructure their entire config. This must require a second click to confirm and triggers an automatic Redis backup before proceeding.

#### F2 — YAML → Plain English (Explain Mode)
- Mod pastes an existing YAML block or loads their full config
- Claude explains each rule in plain English: what triggers it, what action it takes, and any edge cases
- Output is formatted rule-by-rule for readability
- Powered by Haiku 4.5 (fast, low cost, more than sufficient for summarization)

#### F3 — Pattern-Based Config Analyzer (Conflict Checker)
- Mod submits their full AutoMod config
- Claude identifies: duplicate keyword lists, rules with identical triggers, suspicious action ordering, and structurally redundant rules
...
- Powered by `claude-opus-4-7` (reasoning-heavy task, worth the cost — if paying Opus prices, use the strongest model)
...
- On first open, a one-time modal displays: *"ModScript sends your AutoMod configuration to Anthropic's API for processing. This may include usernames or community-specific content in your config. Data is handled per [Anthropic's usage policies](https://www.anthropic.com/policies/usage-policy)."*
...
Because the developer pays for all Anthropic usage under the Model A billing model (see §7.3), the following infrastructure is **MVP-required** to bound cost. These are not user-facing features but they ship as part of the hackathon submission.

- **Per-subreddit-per-day quotas in Redis**, separate per mode. Suggested defaults (tunable via global setting):
  - Generate (Sonnet): **50 calls/day/subreddit**
  - Explain (Haiku): **50 calls/day/subreddit**
  - Conflict Check (Opus): **5 calls/day/subreddit**
...
- **Global kill switch.** A single global setting (`paused: boolean`) the developer can flip via CLI. Server reads it on every Anthropic call and short-circuits with a "temporarily unavailable" message.
...
- **Server-side usage logging.** Token counts (input/output) and approximate cost per call are written to Redis keyed by subreddit and day. No UI required for hackathon — this is data for the developer to audit cost and tune quotas.
...
| AI — Explain Mode | `claude-haiku-4-5-20251001` (fast, low cost, sufficient for summarization) |
| AI — Generate Mode | `claude-sonnet-4-6` (balance of quality and cost for YAML generation) |
| AI — Conflict Analyzer | `claude-opus-4-7` (reasoning-heavy task; strongest model for hardest task) |
| Storage | Devvit Redis via `redis` from `@devvit/web/server` |
| Reddit API | `reddit` and `context` from `@devvit/web/server` |
| External API | Anthropic API via server-side `fetch()` |
...
│  ┌──────────┐ ┌───────┐ ┌────────┐ │
│  │ claude.ts│ │wiki.ts│ │redis.ts│ │
│  │ (core)   │ │ (core)│ │ (core) │ │
│  └────┬─────┘ └───┬───┘ └───┬────┘ │
└───────│────────────│──────────│──────┘
        │            │          │
   Anthropic     reddit         redis
...
└── src/
    ...
    │       ├── claude.ts        # (new) Anthropic API calls + prompts
...
### 5.5 Claude System Prompt Strategy
...
Each mode uses a different model to balance cost and quality:

| Mode | Model | Rationale |
|---|---|---|
| Explain | `claude-haiku-4-5-20251001` | Summarization is well within Haiku's capability; keeps cost low for the most-used mode |
| Generate | `claude-sonnet-4-6` | YAML generation benefits from stronger instruction-following |
| Conflict Analyzer | `claude-opus-4-7` | Structural reasoning across a full config warrants the strongest available model; if paying Opus prices, use the best one |
...
| `devvit.json` `permissions` | config | Declare required capabilities — `redis: true`, `reddit: true`, `http: ["api.anthropic.com"]` |
| `devvit.json` `settings.global` | config | Declare the developer's Anthropic API key as a global secret (`isSecret: true`) plus quota tunables and the `paused` kill switch |
...
| `settings.get('anthropicApiKey')` | `@devvit/web/server` | Read the developer's global Anthropic secret on the server only — never returned to the client |
...
| `fetch()` | global | Server-side calls to `api.anthropic.com` (must be allow-listed in `permissions.http`) |
...
`api.anthropic.com` is **not** in Devvit's globally pre-approved domain list (which includes `api.openai.com` and `generativelanguage.googleapis.com` but not Anthropic). This means the first `devvit upload` triggers manual admin review for the domain. See §7.3 and §10 for timeline implications.
...
1. Mod installs app from App Directory  (no API key entry — the developer's
                                          shared Anthropic key is built in)
...
- **API billing:** The developer (Ben Silva) provides a single Anthropic API key, stored as a Devvit **global secret** (`isSecret: true`). Set once via `npx devvit settings set anthropicApiKey`. It is encrypted by Devvit, never visible in any UI, never returned to the client, never logged. Every install of the app uses this same key — moderators do not enter or manage their own key.
...
- Anthropic API calls require `api.anthropic.com` to be listed under `permissions.http` in `devvit.json`.
- `api.anthropic.com` is **not** on Devvit's globally pre-approved list, so the first upload triggers Reddit admin review of the domain. This must be initiated on Week 1 day one to parallelize the review with feature work.
- All external calls run **server-side only** — the client iframe is network-locked to the app's own domain and cannot reach `api.anthropic.com` directly.
...
- **Max request time: 30 seconds.** Every Hono handler must complete within 30 seconds — including the Anthropic round-trip. Conflict Check on long configs is the most likely to exceed this; mitigated by the F11 max-input-token cap and (if needed) by chunking the analysis. Streaming UIs are not supported because the handler still has to return within 30 seconds.
...
- **Privacy Policy + Terms of Service URLs** are required in app details for any app that uses external `fetch`. Mandatory submission item.
...
- **Privacy disclosure (F5)** stays as written: AutoMod configs may contain banned usernames, flagged phrases, or community-specific content. The disclosure informs mods their config is sent to Anthropic per Anthropic's usage policies. Do not assert specific data retention behavior — that's governed by Anthropic, not this app.
...
- Generated YAML is not validated against the live AutoMod parser (no public API exists for this). Claude is prompted to self-validate, and UI copy advises mods to test on a low-traffic post before relying on new rules in production.
...
|---|---|---|
| Week 1 | Apr 29 – May 5 | Devvit Web project scaffolded (done — React + Hono template in place); `permissions` block added to `devvit.json` and **`api.anthropic.com` submitted for admin review on day 1** (parallelize review with feature work); mod-tools menu item registered and routed to the expanded view; `/api/init` end-to-end with iframe verifying `context.subredditName`; wiki read/write functional with `runAs: USER` verification; basic Claude call (Generate mode) returning YAML in UI; global secret `anthropicApiKey` set via CLI |
...
| Week 4 | May 20 – May 27 | Testing on real subreddits with quotas live; polish; **README with required "Fetch Domains" section**; **Privacy Policy + Terms of Service URLs published and added to app details**; Devpost submission; demo video recorded per shot list below; final verification that `api.anthropic.com` review has been approved before publish |
...
- **Save rate:** % of sessions where mod clicks "Save to Wiki" (proxy for rule quality — if Claude's output is good, mods will save it)
- **Time-to-first-save:** median time from first message to first successful wiki write
- App installed in subreddits within 30 days, **bounded by the cost ceiling** of the developer's shared Anthropic key (see below)
- Qualifies for Reddit Developer Funds engagement milestones

**Note on adoption vs. cost-ceiling tension (Model A):** Under Model A, every install increases the developer's Anthropic bill, which is bounded only by F11 quotas. At the default quotas (5 Conflict Check / 50 Generate / 50 Explain per sub per day) and current Anthropic pricing, the per-subreddit worst-case daily cost is dominated by Conflict Check (~$1.50/sub/day at 5 max-sized runs, well under in normal use). The hackathon target installation count should be set against an explicit monthly budget the developer is willing to absorb. If adoption outpaces budget, the next steps are (in order):
...
|---|---|---|
| Claude API latency exceeds Devvit's 30-second handler timeout (especially Opus on long configs) | Medium | F11 max-input-token cap rejects oversized configs before any API call; tune Conflict Check prompt for brevity; if a single call still risks timeout, chunk the analysis into multiple smaller calls. **Streaming UIs are not viable** — handler must complete in 30s regardless. |
| `api.anthropic.com` admin review takes longer than expected and blocks publish | Medium | Submit the domain in `permissions.http` on Week 1 day one with a stub upload; check status weekly; have a fallback message ready if the review window threatens the submission deadline |
| Cost runaway on developer's shared Anthropic key (F11 quotas misbehave or are bypassed) | Medium | Quotas are atomic Redis `INCR` operations; usage is logged server-side per subreddit per day; global `paused` kill switch can stop all calls instantly; review token-usage logs daily during the hackathon window |
| Claude API latency makes UI feel slow (within timeout) | Medium | Show typing indicator on every streaming response; pre-render the YAML code panel skeleton |
...
| Generated YAML has syntax errors | Low | Claude prompted to self-validate; UI advises testing on low-traffic post before relying on rule in production |
...
| Claude rewrites config instead of appending | Low | Append-only is the default and enforced in the system prompt; rewrite mode requires explicit confirmation and triggers a backup first |
| Privacy Policy / ToS URL not ready by submission deadline | Low | Draft both in Week 3; host on a free static page (GitHub Pages or similar); link in app details before final upload |
...
- **Model A.** Single global Anthropic secret set by the developer via CLI.
...
The smallest possible monetization-adjacent change: let heavy users plug in their own Anthropic key per subreddit. Reduces the developer's marginal cost without introducing a payment system.

- Add a subreddit-scoped setting `anthropicApiKeyOverride` (plain string, **not** an `isSecret` setting — Devvit secrets cannot be subreddit-scoped). Documented honestly in the privacy disclosure as visible to other mods of that subreddit.
...
Conflict Check on Opus is the dominant cost concentration (10–50× the cost of Generate or Explain). If the app sees real adoption, the simplest sustainable design is to gate just this one mode behind a Devvit In-App Purchase.
